...
from merriv.reports.models import MCRStatus, ModelChangeReport
...
def render_junit(report: ModelChangeReport) -> str:
    ...
    if not findings:
        ElementTree.SubElement(
            suite,
            "testcase",
            {"classname": "merriv.gate", "name": "release-decision", "time": "0"},
        )
    for finding in findings:
        case = ElementTree.SubElement(
            suite,
            "testcase",
            {"classname": "merriv.gate", "name": _xml_text(finding.rule_id), "time": "0"},
        )
    ...
    rules = [
        {
            ...
            "properties": {"merrivStatus": finding.status.value},
        }
    ...
    results = [
        {
            ...
            "properties": {
                "merrivStatus": finding.status.value,
