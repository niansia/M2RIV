...
from m2riv.adapters import FakeAdapter
...
from m2riv.reports import ReportBundle, write_report_bundle
...
def run_rare_slice_demo(destination: Path, *, resamples: int = 2_000) -> DemoResult:
    ...
    baseline_path.write_bytes(b"m2riv-demo-fp16")
    candidate_path.write_bytes(b"m2riv-demo-int4")
    ...
    def compare() -> ReleaseComparison:
        return compare_exact_match(
            ...
            baseline_adapter_fingerprint="m2riv.fake@1/fp16",
            candidate_adapter_fingerprint="m2riv.fake@1/int4",
            resamples=resamples,
        )
